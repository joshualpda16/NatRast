...
#--------------------------------------------------------------------------#
###################### Setup Theme product custom fields ######################
#--------------------------------------------------------------------------#
if (file_exists('woocommerce/woocommerce.php')) {
if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
	include (get_template_directory() . "/lib/theme-product-custom-fields.php");
}
}
#--------------------------------------------------------------------------#
...
add_image_size( 'max-size', 700, 9999, true);
...
function kingsize_widgets_init() {
	...
	register_sidebar( array(
		...
		'description' => 'Sidebar specific to WooCommerce when installed and active.',
		'before_widget' => '<div id="%1$s" class="widget-container %2$s">',
		'after_widget' => '</div>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
...
function wm_the_password_form() {
    ...
    $output = '<div class="password-protected"><form action="' . get_option('siteurl') . '/wp-login.php?action=postpass" method="post">
    <p><strong>' . __("This is a password protected page with private content.", "kslang") . '</strong></p>
    <p><div for="' . $label . '" style="padding-top:10px;">' . __("Enter your password below:", "kslang") . '</div><div style="padding-top:10px;"> <input name="post_password" id="' . $label . '" type="password" size="20" class="" /> <input type="submit" name="Submit" class="send-link" value="' . esc_attr__("Login") . '" /></div></p>
    </form></div>';
...
function language_selector_flags(){
    ...
    if(!empty($languages)){
        foreach($languages as $l){
            ...
            echo '<img src="'.$l['country_flag_url'].'" height="18" alt="'.$l['language_code'].'" width="20" />';
            if(!$l['active']) echo '</a>';
        }
    }
}
