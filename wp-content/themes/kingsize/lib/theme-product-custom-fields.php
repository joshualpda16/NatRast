...
/**
 * @KingSize 2011-2016
 * The PHP code for setup Theme product custom fields.
 * OurWebMedia https://www.ourwebmedia.com
 **/
include_once("meta-fields.php");

/*
	Begin creating custom product fields 
*/

$general_postmetas = 
	array (
		/*
			Begin product custom fields
		*/

		array('type' => 'start'),
		
		array("section" => "Disable the Sidebar", "name" => "Disable the Sidebar", "id" => "post_sidebar_hide",  "title" => "Hide the Sidebar", 'type' => 'checkbox', 'desc'=>'If checked will hide the sidebar on this product.'),
		...
		array("section" => "Grid Overlay", "name" => "Grid Overlay", "id" => "kingsize_post_grid_overlay", "type" => "select", "title" => "Grid Overlay", "items" => array( ""=>"Default" , "grid_disabled"=>"Disable Grid Overlay" , "grid_enabled"=>"Enable Grid Overlay"),'desc'=>'Control the grid overlay on this product.' ,'show_default_title'=>"false"),
...
function general_product_meta_box() {
...
function background_product_meta_box() {
...
function video_product_meta_box() {

	global $video_postmetas,$post;

	echo '<p style="padding:10px 0 0 0;">'.__('Manage and customize your background video options here specific to this product. Make sure you\'ve enabled the Video Background found inside the "KingSize Post Background Options" above.', 'framework').'</p>';
	// Use nonce for verification
	echo '<input type="hidden" name="myplugin_noncename" id="myplugin_noncename" value="' . wp_create_nonce( plugin_basename(__FILE__) ) . '" />';
...
function slider_product_meta_box() {

	global $slider_postmetas,$post;

	echo '<p style="padding:10px 0 0 0;">'.__('Manage and customize your background slider image options here specific to this product. Make sure you\'ve enabled the Image Slider Background found inside the "KingSize product Background Options" above.', 'framework').'</p>';
	// Use nonce for verification
	echo '<input type="hidden" name="myplugin_noncename" id="myplugin_noncename" value="' . wp_create_nonce( plugin_basename(__FILE__) ) . '" />';
...
function product_save_postdata( $post_id,$post_postmetas=array() ) {
	...
	if ( 'product' != $_POST['post_type'] || !isset($_POST['post_type'])) return $post_id; //Fix for quick edit mode.
	...
	foreach ( $post_postmetas as $postmeta ) {
		...
		if ($_POST[$postmeta['id']]) {
			product_update_custom_meta($post_id, $_POST[$postmeta['id']], $postmeta['id']);
		}
...
function product_update_custom_meta($postID, $newvalue, $field_name) {
...
function product_create_meta_box() {
	if ( function_exists('add_meta_box')) {  
		add_meta_box( 'general_metabox', 'KingSize General product Options', 'general_product_meta_box', 'product', 'normal', 'high' );
		add_meta_box( 'background_metabox', 'KingSize product Background Options', 'background_product_meta_box', 'product', 'normal', 'high' );
		add_meta_box( 'slider_metabox', 'KingSize Image Slider Options', 'slider_product_meta_box', 'product', 'normal', 'high' );
		add_meta_box( 'video_metabox', 'KingSize Video Background Options', 'video_product_meta_box', 'product', 'normal', 'high' );
	}
} 
...
function product_save($post_id)
{
	...
	product_save_postdata( $post_id,$general_postmetas);
	product_save_postdata( $post_id,$background_postmetas);
	product_save_postdata( $post_id,$slider_postmetas);
	product_save_postdata( $post_id,$video_postmetas);
	
}

//init
add_action('admin_menu', 'product_create_meta_box'); 
